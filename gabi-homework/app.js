...
(()=>{
  ...
  third.querySelector('.tag').textContent='LESSON 3 · HOMEWORK · 1 SEP';
  third.querySelector('h3').textContent='The Hollow · Articles · Smells';
  third.querySelector('p').textContent='Quizlet vocabulary, music-festival articles and strong-smell language.';
  const btn3=third.querySelector('.open-btn');
  btn3.id='openHw3';
  btn3.textContent='Homework →';
  btn3.onclick=()=>{window.location.href='lesson-3-homework.html'};
  grid.appendChild(third);
  const mq=window.matchMedia('(max-width: 900px)');
  const apply=()=>grid.style.gridTemplateColumns=mq.matches?'1fr':'repeat(3,minmax(0,1fr))';
  apply();
